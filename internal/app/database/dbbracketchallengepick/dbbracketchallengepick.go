...
func Update(s *BracketChallengePick) error {
	sql := database.BuildUpdate("bracket_challenge_pick", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("bracket_challenge_pick: couldn't update %s", err)
	}

	return nil
}

func Save(s *BracketChallengePick) error {
	if s.BracketChallengePickID > 0 {
		return Update(s)
	} else {
		return Insert(s)
	}
