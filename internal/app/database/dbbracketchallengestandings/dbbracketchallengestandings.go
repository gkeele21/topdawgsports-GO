...
func Update(s *BracketChallengeStandings) error {
	sql := database.BuildUpdate("bracket_challenge_standings", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("bracket_challenge_standings: couldn't update %s", err)
	}
...
func Save(s *BracketChallengeStandings) error {
	if s.BracketChallengeStandingsID > 0 {
		return Update(s)
	} else {
		return Insert(s)
	}
