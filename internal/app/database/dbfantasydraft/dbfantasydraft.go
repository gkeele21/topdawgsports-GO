...
func Update(s *FantasyDraft) error {
	sql := database.BuildUpdate("fantasy_draft", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("fantasy_draft: couldn't update %s", err)
	}

	return nil
}

func Save(s *FantasyDraft) error {
	if s.FantasyDraftID > 0 {
		return Update(s)
	} else {
		return Insert(s)
	}
