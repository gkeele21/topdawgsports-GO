...
func Update(s *FantasyGameSetting) error {
	sql := database.BuildUpdate("fantasy_game_setting", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("fantasy_game_setting: couldn't update %s", err)
	}
