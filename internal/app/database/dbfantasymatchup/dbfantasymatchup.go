...
func Update(s *FantasyMatchup) error {
	sql := database.BuildUpdate("fantasy_matchup", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("fantasy_matchup: couldn't update %s", err)
	}

	return nil
}

func Save(s *FantasyMatchup) error {
	if s.FantasyMatchupID > 0 {
		return Update(s)
	} else {
		return Insert(s)
	}
