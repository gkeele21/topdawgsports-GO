...
func Update(s *FantasyPlayerValue) error {
	sql := database.BuildUpdate("fantasy_player_value", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("fantasy_player_value: couldn't update %s", err)
	}

	return nil
}

func Save(s *FantasyPlayerValue) error {
	if s.FantasyPlayerValueID > 0 {
		return Update(s)
	} else {
		return Insert(s)
	}
