...
func Update(s *FantasySettingOption) error {
	sql := database.BuildUpdate("fantasy_setting_option", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("fantasy_setting_option: couldn't update %s", err)
	}

	return nil
}

func Save(s *FantasySettingOption) error {
	if s.FantasySettingOptionID > 0 {
		return Update(s)
	} else {
		return Insert(s)
	}
