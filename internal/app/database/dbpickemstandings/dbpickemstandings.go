...
func Update(s *PickEmStandings) error {
	sql := database.BuildUpdate("pickem_standings", s)
	_, err := database.Exec(sql, database.GetArgumentsForUpdate(*s)...)

	if err != nil {
		return fmt.Errorf("pickem_standings: couldn't update %s", err)
	}
